...
namespace BaseProj.SharpVectors.SharpVectorRenderingWpf.Utils
{
    public sealed class WpfCacheManager : ICacheManager
    {
        ...
        private readonly string cacheDocPath;
        ...
        {
            var cacheElm = GetCacheElm(uri);

            var expires = DateTime.MinValue;
            if (cacheElm.HasAttribute("expires")) expires = DateTime.Parse(cacheElm.GetAttribute("expires"));

            var lastModified = DateTime.MinValue;
            if (cacheElm.HasAttribute("last-modified"))
                lastModified = DateTime.Parse(cacheElm.GetAttribute("last-modified"));

            var cachedUri = GetLocalPathUri(cacheElm);

            return new CacheInfo(expires, cacheElm.GetAttribute("etag"), lastModified, cachedUri,
                cacheElm.GetAttribute("content-type"));
        }
        ...
        public void SetCacheInfo(Uri uri, CacheInfo cacheInfo, Stream stream)
        {
            ...
            if (stream != null)
            {
                ...
                var fs = File.OpenWrite(Path.Combine(cacheDir, localPath));
        ...
        {
            if (cacheDoc == null) cacheDoc = new XmlDocument();
        ...
        {
            if (cacheDoc == null) return;
            cacheDoc.Save(cacheDocPath);
        }

        private XmlElement GetCacheElm(Uri uri)
        {
            if (uri == lastUri && lastCacheElm != null) return lastCacheElm;

            //string xpath = "/cache/resource[@url='" + uri.ToString() + "']";
            var xpath = "/cache/resource[@url='" + uri.ToString().Replace("'", "&apos;") + "']";
            var node = cacheDoc.SelectSingleNode(xpath);
            if (node != null)
            {
        ...
        private Uri GetLocalPathUri(XmlElement cacheElm)
        {
            if (cacheElm.HasAttribute("local-path"))
            {
                var path = Path.Combine(cacheDir, cacheElm.GetAttribute("local-path"));
                if (File.Exists(path))
                {
