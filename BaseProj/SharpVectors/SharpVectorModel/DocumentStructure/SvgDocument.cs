...
using BaseProj.SharpVectors.SharpVectorCore.Utils.Xml;
using BaseProj.SharpVectors.SharpVectorCss.Css;
using BaseProj.SharpVectors.SharpVectorModel.BasicTypes;
using BaseProj.SharpVectors.SharpVectorModel.Events;
using BaseProj.SharpVectors.SharpVectorModel.Resources;
...
namespace BaseProj.SharpVectors.SharpVectorModel.DocumentStructure
{
    ...
    ///         <see href="http://www.w3.org/TR/REC-DOM-Level-1/level-one-html.html">
    ///             Document
    ///             Object Model (HTML) Level 1
    ///         </see>
    ///         chapter of the
    ...
    public class SvgDocument : CssXmlDocument, ISvgDocument
    {
        ...
        public ISvgWindow Window => _window;
        ...
        {
            var result = SvgElementFactory.Create(prefix, localName, ns, this);
            if (result != null) return result;

            if (ns == SvgNamespace) return new SvgElement(prefix, localName, ns, this);
        ...
        private readonly SvgWindow _window;
            ...
            : this()
        {
            ...
            AddStyleElement(SvgNamespace, "style");
        ...
        public XmlNamespaceManager NamespaceManager
        {
            get
            {
                if (namespaceManager == null)
                {
                    ...
                    namespaceManager.AddNamespace(string.Empty, SvgNamespace);
                    namespaceManager.AddNamespace("svg", SvgNamespace);
                    namespaceManager.AddNamespace("xlink", XLinkNamespace);
                }
        ...
        {
            foreach (var supportedFeature in supportedFeatures)
        ...
        {
            if (CustomSettings != null) return CustomSettings;
            var xmlResolver = new DynamicXmlUrlResolver();
        ...
        public override void Load(string url)
        {
            ...
            var fileUrl = new UriBuilder(url);
            ...
            using (var reader = CreateValidatingXmlReader(url))
            {
                Load(reader);
            }
        }
        ...
        public void Load(string baseUrl, Stream stream)
        {
            ...
            using (var reader = CreateValidatingXmlReader(baseUrl, stream))
            {
                Load(reader);
            }
        }
        ...
        public override void Load(TextReader reader)
        {
            ...
            using (var xmlReader = CreateValidatingXmlReader(reader))
            {
                Load(xmlReader);
            }
        }
        ...
        public override void Load(Stream stream)
        {
            ...
            using (var reader = CreateValidatingXmlReader(string.Empty, stream))
            {
                Load(reader);
            }
        }
        ...
        private static string GetEntityUri(string uri)
        {
            lock (_entitiesUrisLock)
            {
                if (_entitiesUris == null)
                {
                    ...
                    var names = _rootType.Assembly.GetManifestResourceNames();
        ...
        {
            var fullPath = absoluteUri.ToString();
            if (!string.IsNullOrEmpty(fullPath))
            {
                ...
                {
                    var resourceUri = GetEntityUri("www.w3.org.Graphics.SVG.1.1.DTD.svg11-basic.dtd");
                    if (resourceUri != null)
                ...
                {
                    var resourceUri = GetEntityUri("www.w3.org.Graphics.SVG.1.1.DTD.svg11.dtd");
                    if (resourceUri != null)
                        return GetEntityFromUri(resourceUri, ofObjectToReturn);
                }
                ...
                {
                    var resourceUri = GetEntityUri("www.w3.org.TR.2001.REC-SVG-20010904.DTD.svg10.dtd");
                    if (resourceUri != null)
                        return GetEntityFromUri(resourceUri, ofObjectToReturn);
                }
                ...
                {
                    var resourceUri = GetEntityUri("www.w3.org.Graphics.SVG.1.1.DTD.svg11-tiny.dtd");
                    if (resourceUri != null)
                        return GetEntityFromUri(resourceUri, ofObjectToReturn);
                }
            }
            ...
            var path = absoluteUri.Host + absoluteUri.AbsolutePath.Replace('/', '.');
            var foundResource = GetEntityUri(path);
            ...
            if (ofObjectToReturn == typeof(Stream))
                using (var resourceStream = _rootType.Assembly.GetManifestResourceStream(path))
                {
                    if (resourceStream != null)
                    {
                        ...
                        var resourceLength = (int)resourceStream.Length;
                        var memoryStream = new MemoryStream(resourceLength);
        ...
        private string OnXmlResolverResolving(string relativeUri)
        {
            ...
            {
                var e = new SvgResolveUriEventArgs { Uri = relativeUri };
                ResolvingUri(this, e);
                relativeUri = e.Uri;
            }
        ...
        private XmlParserContext GetXmlParserContext()
        {
            ...
            var xmlParserContext = new XmlParserContext(null,
        ...
        private string OnResolveXmlNamespaceManager(string prefix)
        {
            ...
            {
                var e = new SvgResolveNamespaceEventArgs(prefix);
        ...
        private XmlReader CreateValidatingXmlReader(string uri, Stream stream)
        {
            ...
            var xmlReaderSettings = GetXmlReaderSettings();
            if (string.IsNullOrEmpty(uri)) return XmlReader.Create(stream, xmlReaderSettings, uri);
        ...
        public XmlNode GetNodeByUri(string absoluteUrl)
        {
            ...
            if (absoluteUrl.StartsWith("#")) return GetElementById(absoluteUrl.Substring(1));

            var docUri = ResolveUri("");
            ...
            if (fragment.Length == 0)
            {
                ...
                if (docUri.AbsolutePath == absoluteUri.AbsolutePath) return this;

                var doc = new SvgDocument((SvgWindow)Window);
...

            {
                ...
                var noFragment = absoluteUri.AbsoluteUri.Replace(fragment, "");
                var doc = (SvgDocument)GetNodeByUri(new Uri(noFragment));
        ...
        {
            var workingDir = _window.WorkingDir;

            var baseUri = BaseURI;
            if (baseUri.Length == 0) baseUri = "file:///" + workingDir.FullName.Replace('\\', '/');
        ...
        public string Title
        {
            get
            {
                ...
                var node = SelectSingleNode("/svg:svg/svg:title[text()!='']", NamespaceManager);
        ...
        /// <summary>
        ///     The domain name of the server that served the document, or a null string if the server cannot be identified by a
        ///     domain name.
        /// </summary>
        ...
        /// </summary>
        public ISvgSvgElement RootElement => DocumentElement as ISvgSvgElement;
        ...
        public override XmlElement GetElementById(string elementId)
        {
            ...
            // Find the item
            if (collectedIds.ContainsKey(elementId)) return collectedIds[elementId];
