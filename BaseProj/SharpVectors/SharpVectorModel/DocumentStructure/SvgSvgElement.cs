...
namespace BaseProj.SharpVectors.SharpVectorModel.DocumentStructure
{
    ...
    public sealed class SvgSvgElement : SvgTransformableElement, ISvgSvgElement
    {
        ...
        public bool IsOuterMost => ParentNode is SvgDocument;
        ...
        public ISvgAnimatedLength X
        {
            get
            {
                if (x == null) x = new SvgAnimatedLength(this, "x", SvgLengthDirection.Horizontal, "0px");
                return x;
            }
        }
        ...
        public ISvgAnimatedLength Y
        {
            get
            {
                if (y == null) y = new SvgAnimatedLength(this, "y", SvgLengthDirection.Vertical, "0px");
                return y;
            }
        }
        ...
        private string widthAsString
        {
            get
            {
                var ownerWindow = (SvgWindow)OwnerDocument.Window;
            ...
            double defaultValue, SvgLengthDirection dir)
        {
            ...
            if (inValue.Length > 0)
            {
                if (inValue.EndsWith("%"))
                {
                    var perc = SvgNumber.ParseNumber(inValue.Substring(0, inValue.Length - 1)) / 100;
        ...
        /// <summary>
        ///     The position and size of the viewport (implicit or explicit) that corresponds to this 'svg' element. When the user
        ///     agent is actually rendering the content, then the position and size values represent the actual values when
        ///     rendering. The position and size values are unitless values in the coordinate system of the parent element. If no
        ///     parent element exists (i.e., 'svg' element represents the root of the document tree), if this SVG document is
        ///     embedded as part of another document (e.g., via the HTML 'object' element), then the position and size are unitless
        ///     values in the coordinate system of the parent document. (If the parent uses CSS or XSL layout, then unitless values
        ///     represent pixel units for the current CSS or XSL viewport, as described in the CSS2 specification.) If the parent
        ///     element does not have a coordinate system, then the user agent should provide reasonable default values for this
        ///     attribute.
        ...
        public ISvgRect Viewport
        {
            get
            {
                if (viewport == null)
                {
                    var calcParentVPWidth = ViewportElement == null
                        ? OwnerDocument.Window.InnerWidth
                        : ((ISvgFitToViewBox)ViewportElement).ViewBox.AnimVal.Width;

                    var calcParentVPHeight = ViewportElement == null
                        ? OwnerDocument.Window.InnerHeight
                        : ((ISvgFitToViewBox)ViewportElement).ViewBox.AnimVal.Height;

                    var x = getViewportProp("x", GetAttribute("x"), calcParentVPWidth, 0,
                        SvgLengthDirection.Horizontal);
                    var y = getViewportProp("y", GetAttribute("y"), calcParentVPHeight, 0, SvgLengthDirection.Vertical);
                    var width = getViewportProp("width", widthAsString, calcParentVPWidth,
                        OwnerDocument.Window.InnerWidth, SvgLengthDirection.Horizontal);
                    var height = getViewportProp("height", heightAsString, calcParentVPHeight,
                        OwnerDocument.Window.InnerHeight, SvgLengthDirection.Vertical);
        ...
        ///     the user agent actually knows the physical size of a "screen unit", this attribute will express that information;
        ...
        /// <summary>
        ///     The definition of the initial view (i.e., before magnification and panning) of the current innermost SVG document
        ///     fragment. The meaning depends on the situation:
        ///     * If the initial view was a "standard" view, then:
        ///     o the values for viewBox, preserveAspectRatio and zoomAndPan within currentView will match the values for the
        ///     corresponding DOM attributes that are on SVGSVGElement directly
        ...
        ///     o the values for viewBox, preserveAspectRatio and zoomAndPan within currentView will correspond to the
        ///     corresponding attributes for the given 'view' element
        ///     o the values for transform and viewTarget within currentView will be null
        ///     * If the initial view was a link into another element (i.e., other than a 'view'), then:
        ///     o the values for viewBox, preserveAspectRatio and zoomAndPan within currentView will match the values for the
        ///     corresponding DOM attributes that are on SVGSVGElement directly for the closest ancestor 'svg' element
        ...
        ///     * If the initial view was a link into the SVG document fragment using an SVG view specification fragment identifier
        ///     (i.e., #svgView(...)), then:
        ///     o the values for viewBox, preserveAspectRatio, zoomAndPan, transform and viewTarget within currentView will
        ///     correspond to the values from the SVG view specification fragment identifier
        ///     The object itself and its contents are both readonly.
        /// </summary>
        private ISvgViewSpec currentView;
        ...
        public ISvgViewSpec CurrentView
        {
            ...
            {
                if (currentView == null)
                    currentView = new SvgViewSpec(this);
        ...
        /// <summary>
        ///     This attribute indicates the current scale factor relative to the initial view to take into account user
        ///     magnification and panning operations, as described under Magnification and panning. DOM attributes currentScale and
        ///     currentTranslate are equivalent to the 2x3 matrix [a b c d e f] = [currentScale 0 0 currentScale currentTranslate.x
        ///     currentTranslate.y]. If "magnification" is enabled (i.e., zoomAndPan="magnify"), then the effect is as if an extra
        ///     transformation were placed at the outermost level on the SVG document fragment (i.e., outside the outermost 'svg'
        ///     element).
        /// </summary>
        /// <exception cref="DomException">
        ///     NO_MODIFICATION_ALLOWED_ERR: Raised on an attempt to change the value of a readonly
        ...
        private ISvgMatrix cachedViewBoxTransform;
        ...
        public ISvgMatrix ViewBoxTransform
        {
            ...
            get
            {
                if (cachedViewBoxTransform == null)
                {
                    ...
                    var attrWidth = Width.AnimVal.Value;
                    var attrHeight = Height.AnimVal.Value;
                    ...
                    {
                        var r = CurrentView.ViewBox.AnimVal;
                        x += -r.X;
                        y += -r.Y;
                        w = r.Width;
                        h = r.Height;
                    ...
                    var x_ratio = attrWidth / w;
                    var y_ratio = attrHeight / h;

                    var par = CurrentView.PreserveAspectRatio.AnimVal;
                    ...
                    else
                    {
                        ...
                        double x_trans = 0;
                        var x_diff = attrWidth - x_ratio * w;
                        double y_trans = 0;
                        var y_diff = attrHeight - x_ratio * h;
                    ...
                    if (this == OwnerDocument.RootElement)
                    {
                        matrix = matrix.Translate(currentTranslate.X, currentTranslate.Y);
                        matrix = matrix.Scale(currentScale);
                    }
        ...
        public ISvgPoint CurrentTranslate
        {
            get
            {
                if (this == OwnerDocument.RootElement)
                {
                    if (currentTranslate == null) currentTranslate = CreateSvgPoint();
        ...
        private readonly List<Timer> redrawTimers = new List<Timer>();
        ...
        /// </param>
        /// <returns>
        ///     A number which acts as a unique identifier for the given suspendRedraw() call. This value must be passed as
        ///     the parameter to the corresponding unsuspendRedraw() method call.
        /// </returns>
        public int SuspendRedraw(int maxWaitMilliseconds)
        {
            if (maxWaitMilliseconds > 60000)
                maxWaitMilliseconds = 60000;
            var t = new Timer(maxWaitMilliseconds);
            t.AutoReset = false;
            t.Elapsed += RedrawTimerElapsed;
            t.Enabled = true;
            redrawTimers.Add(t);
        ...
        ///     A number which acts as a unique identifier for the desired suspendRedraw() call. The
        ///     number supplied must be a value returned from a previous call to suspendRedraw()
        /// </param>
        /// <exception cref="DomException">
        ///     This method will raise a DOMException with value NOT_FOUND_ERR if an invalid value
        ///     (i.e., no such suspend_handle_id is active) for suspend_handle_id is provided.
        /// </exception>
        public void UnsuspendRedraw(int suspendHandleId)
        {
            Timer timer = null;
            foreach (var t in redrawTimers)
            ...
            if (OwnerDocument.Window.Renderer.InvalidRect != SvgRectF.Empty)
                OwnerDocument.Window.Renderer.Render(OwnerDocument);
        }
        ...
        {
            foreach (var t in redrawTimers) t.Enabled = false;
            redrawTimers.Clear();
            if (OwnerDocument.Window.Renderer.InvalidRect != SvgRectF.Empty)
                OwnerDocument.Window.Renderer.Render(OwnerDocument);
        }
