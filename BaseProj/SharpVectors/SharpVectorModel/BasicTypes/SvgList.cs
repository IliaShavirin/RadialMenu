...
namespace BaseProj.SharpVectors.SharpVectorModel.BasicTypes
{
    ...
    ///     Note we're using <see cref="List{T}" /> (as opposed to deriving from) to hide unneeded <see cref="List{T}" />
    ///     methods
    ...
    public abstract class SvgList<T> : IEnumerable<T>
    {
        ...
        private void RemoveItem(T item)
        {
            ...
            if (index >= 0) RemoveItem((uint)index);
        ...
        public T InsertItemBefore(T newItem, uint index)
        {
            ...
            if (itemOwnerMap.ContainsKey(newItem))
                ((SvgList<T>)itemOwnerMap[newItem]).RemoveItem(newItem);

            // insert item into this list
            items.Insert(i, newItem);

            // update the itemOwnerMap to associate newItem with this list
            itemOwnerMap[newItem] = this;
