...
namespace BaseProj.SharpVectors.SharpVectorConverters.Utils
{
    ...
    public sealed class DirectoryCopier
    {
        ...
        public int Copy(string sourceDir, string targetDir)
        {
            ...
            if (sourceDir.Length == 0) throw new ArgumentException("sourceDir");
            if (!Directory.Exists(sourceDir)) throw new InvalidOperationException();

            if (targetDir == null) throw new ArgumentNullException("targetDir");
            if (targetDir.Length == 0) throw new ArgumentException("targetDir");
            ...
            if (_includeSecurity) dirSecurity = sourceInfo.GetAccessControl();
            ...
            {
                if (dirSecurity != null) targetInfo.SetAccessControl(dirSecurity);
            }
        ...
        {
            var listInfo = source.GetFiles();

            var fileCount = listInfo == null ? 0 : listInfo.Length;
            ...
            for (var i = 0; i < fileCount; i++)
            {
                ...
                if (_includeSecurity) File.SetAccessControl(filePath, fi.GetAccessControl());
            }
        }
