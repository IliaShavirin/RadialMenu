...
namespace BaseProj.SharpVectors.SharpVectorCore.Svg
{
    ...
    public struct SvgPointF : IEquatable<SvgPointF>
    {
        ...
        {
            if (left.X.Equals(right.X)) return left.Y.Equals(right.Y);
        ...
        {
            if (obj is SvgPointF) return Equals((SvgPointF)obj);
        ...
        {
            if (other.X == X && other.Y == Y) return other.GetType().Equals(GetType());
