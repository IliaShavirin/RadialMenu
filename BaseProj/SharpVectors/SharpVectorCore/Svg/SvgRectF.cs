...
namespace BaseProj.SharpVectors.SharpVectorCore.Svg
{
    ...
    public struct SvgRectF : IEquatable<SvgRectF>
    {
        ...
        {
            get => new SvgSizeF(_width, _height);
            set
            {
                _width = value.Width;
        ...
        public bool IsEmpty
        {
            get
            {
                if (_width > 0f) return _height <= 0f;
        ...
        {
            if (obj is SvgRectF) return Equals((SvgRectF)obj);
        ...
        {
            if (other.X == Left && other.Y == Top && other.Width == _width) return other.Height == _height;
        ...
        {
            if (Left <= x && x < Left + _width && Top <= y) return y < Top + _height;
        ...
        {
            if (Left <= rect.X && rect.X + rect.Width <=
                Left + _width && Top <= rect.Y)
                return rect.Y + rect.Height <= Top + _height;
        ...
        {
            var result = Intersection(rect, this);
            Left = result.X;
            Top = result.Y;
            _width = result.Width;
        ...
        {
            if (rect.X < Left + _width && Left < rect.X + rect.Width &&
                rect.Y < Top + _height)
                return Top < rect.Y + rect.Height;
        ...
        public override string ToString()
        {
            ...
            return "{X=" + Left.ToString(culture) + ",Y=" + Top.ToString(culture)
        ...
        {
            var dLeft = Math.Max(a.X, b.X);
            var dRight = Math.Min(a.X + a.Width, b.X + b.Width);
            var dTop = Math.Max(a.Y, b.Y);
            var dBottom = Math.Min(a.Y + a.Height, b.Y + b.Height);
        ...
        {
            var dLeft = Math.Min(a.X, b.X);
            var dRight = Math.Max(a.X + a.Width, b.X + b.Width);
            var dTop = Math.Min(a.Y, b.Y);
            var dBottom = Math.Max(a.Y + a.Height, b.Y + b.Height);
