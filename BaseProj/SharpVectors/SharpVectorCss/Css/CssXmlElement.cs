...
namespace BaseProj.SharpVectors.SharpVectorCss.Css
{
    public class CssXmlElement : Element, IElementCssInlineStyle
    {
        ...
        public new CssXmlDocument OwnerDocument => (CssXmlDocument)base.OwnerDocument;
        ...
        {
            if ((feature == "StyleSheets" || feature == "CSS") && version == "2.0") return true;
        ...
        public ICssStyleDeclaration Style
        {
            ...
            {
                if (style == null)
                    style = new CssStyleDeclaration(GetAttribute("style", string.Empty), null, false,
        ...
        public virtual ICssStyleDeclaration GetComputedStyle(string pseudoElt)
        {
            if (cachedCSD == null)
            {
                var csd = new CssCollectedStyleDeclaration(this);
                var currentMedia = OwnerDocument.Media;
        ...
        {
            var csd = GetComputedStyle(pseudoElt);
        ...
        public virtual void AttributeChange(object src, XmlNodeChangedEventArgs args)
        {
            ...
            if (attribute != null) HandleAttributeChange(attribute);
        ...
        {
            if (cssChangeHandler != null) cssChangeHandler();
        }

        protected void FireAttributeChange(object src, XmlNodeChangedEventArgs args)
        {
            if (attributeChangeHandler != null) attributeChangeHandler(src, args);
        }

        protected void FireElementChange(object src, XmlNodeChangedEventArgs args)
        {
            if (elementChangeHandler != null) elementChangeHandler(src, args);
        }

        protected void FireParentNodeChange(object src, XmlNodeChangedEventArgs args, bool fireEvent)
        {
            if (fireEvent && parentNodeChangeHandler != null) parentNodeChangeHandler(src, args);

            foreach (XmlNode child in ChildNodes)
            {
                if (child.NodeType != XmlNodeType.Element) continue;

                var cssChild = child as CssXmlElement;

                if (cssChild != null) cssChild.ParentNodeChange(src, args);
            }
        }

        protected void FireChildNodeChange(object src, XmlNodeChangedEventArgs args, bool fireEvent)
        {
            if (fireEvent && childNodeChangeHandler != null) childNodeChangeHandler(src, args);

            var cssParent = ParentNode as CssXmlElement;

            if (cssParent != null) cssParent.ChildNodeChange(src, args);
        }
