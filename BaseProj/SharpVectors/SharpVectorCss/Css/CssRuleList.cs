...
namespace BaseProj.SharpVectors.SharpVectorCss.Css
{
    ...
    public sealed class CssRuleList : ICssRuleList
    {
         ...
         * }
         * */
        private void Parse(ref string css, object parent, bool readOnly, IList<string> replacedStrings,
            CssStyleSheetType origin)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    var rule = CssStyleRule.Parse(ref css, parent, readOnly, replacedStrings, origin);
                    if (rule != null)
                    {
                        InsertRule(rule);
                    }
                    else
                    {
                        ...
                        var startBracket = css.IndexOf("{");
                        var endBracket = css.IndexOf("}");
                        var endSemiColon = css.IndexOf(";");
                        int endRule;
            ...
            IList<string> replacedStrings, bool readOnly, CssStyleSheetType origin)
        {
            ...
            if (parent is CssRule || parent is CssStyleSheet)
                ...
                throw new Exception(
                    "The CssRuleList constructor can only take a CssRule or CssStyleSheet as it's first argument " +
                    parent.GetType());
        ...
        private readonly List<CssRule> cssRules = new List<CssRule>();
        ...
        internal void GetStylesForElement(XmlElement elt, string pseudoElt, MediaList ml,
            CssCollectedStyleDeclaration csd)
        {
            var len = Length;
            for (ulong i = 0; i < len; i++)
            {
                var csr = (CssRule)this[i];
        ...
        /// </summary>
        public ulong Length => (ulong)cssRules.Count;

        /// <summary>
        ///     Used to retrieve a CSS rule by ordinal index. The order in this collection represents the order of the rules in the
        ///     CSS style sheet. If index is greater than or equal to the number of rules in the list, this returns null
        /// </summary>
        public ICssRule this[ulong index]
        {
            get => index < Length ? cssRules[(int)index] : null;
            set { }
        }
