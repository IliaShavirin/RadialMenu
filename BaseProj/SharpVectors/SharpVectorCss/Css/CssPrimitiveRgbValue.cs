...
namespace BaseProj.SharpVectors.SharpVectorCss.Css
{
    public sealed class CssPrimitiveRgbValue : CssPrimitiveValue
    {
        ...
        private static readonly Regex reColor =
            new Regex("^#([a-f]|[A-F]|[0-9]){3}(([a-f]|[A-F]|[0-9]){3})?$", RegexOptions.Compiled);
        ...
        public static bool IsColorName(string cssText)
        {
            ...
            if (namedColors.ContainsKey(cssText) || reColor.Match(cssText).Success) return true;
