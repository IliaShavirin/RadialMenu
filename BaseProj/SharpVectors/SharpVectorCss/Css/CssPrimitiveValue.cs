...
namespace BaseProj.SharpVectors.SharpVectorCss.Css
{
    ...
    public class CssPrimitiveValue : CssValue, ICssPrimitiveValue
    {
        ...
        private readonly CssRect rectValue;
        ...
        public static CssPrimitiveValue Create(Match match, bool readOnly)
        {
            if (match.Groups["length"].Success)
                return new CssPrimitiveLengthValue(match.Groups["lengthNumber"].Value, match.Groups["lengthUnit"].Value,
                    readOnly);
            if (match.Groups["angle"].Success)
                return new CssPrimitiveAngleValue(match.Groups["angleNumber"].Value, match.Groups["angleUnit"].Value,
                    readOnly);
            ...
            if (match.Groups["colorIdent"].Success &&
                CssPrimitiveRgbValue.IsColorName(match.Groups["colorIdent"].Value))
        ...
        {
            if (double.IsNaN(floatValue)) throw new DomException(DomExceptionType.InvalidAccessErr);
        ...
        {
            get => _primitiveType;
            protected set => _primitiveType = value;
        }
