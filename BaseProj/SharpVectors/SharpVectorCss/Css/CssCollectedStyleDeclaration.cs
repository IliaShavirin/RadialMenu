...
namespace BaseProj.SharpVectors.SharpVectorCss.Css
{
    ...
    public class CssCollectedStyleDeclaration : CssStyleDeclaration
    {
        ...
        private readonly Dictionary<string, CssCollectedProperty> collectedStyles =
        ...
        public void CollectProperty(string name, int specificity, CssValue cssValue, CssStyleSheetType origin,
            string priority)
        {
            var newProp = new CssCollectedProperty(name, specificity, cssValue, origin, priority);
            ...
            {
                var existingProp = collectedStyles[name];
                if (newProp.IsBetterThen(existingProp)) collectedStyles[name] = newProp;
            }
        }
        ...
        public CssStyleSheetType GetPropertyOrigin(string propertyName)
        {
            ...
            {
                var scp = collectedStyles[propertyName];
                return scp.Origin;
            }
        ...
        {
            return collectedStyles.ContainsKey(propertyName) ? collectedStyles[propertyName].Priority : string.Empty;
        }

        private ICssValue getParentStyle(string propertyName)
        {
            var doc = _element.OwnerDocument as CssXmlDocument;
            var parentNode = _element.ParentNode as XmlElement;
            if (doc != null && parentNode != null)
            {
                var parentCsd = doc.GetComputedStyle(parentNode, string.Empty);
                if (parentCsd == null)
                    return null;
        ...
        public override ICssValue GetPropertyCssValue(string propertyName)
        {
            ...
            var doc = (CssXmlDocument)_element.OwnerDocument;

            if (doc.CssPropertyProfile.IsInheritable(propertyName))
            {
                var parValue = getParentStyle(propertyName);
                if (parValue != null) return parValue;
            }

            var initValue = doc.CssPropertyProfile.GetInitialValue(propertyName);
            if (initValue == null)
                return null;
        ...
        {
            var value = (CssValue)GetPropertyCssValue(propertyName);
            if (value != null)
        ...
        /// </summary>
        public override ulong Length => (ulong)collectedStyles.Count;
        ...
        public override string CssText
        {
            get
            {
                ...
                for (ulong i = 0; i < len; i++)
                {
                    ...
                    var prio = GetPropertyPriority(propName);
                    if (prio.Length > 0) sb.Append(" !" + prio);
                    sb.Append(";");
                }

                return sb.ToString();
            }
            set => throw new DomException(DomExceptionType.NoModificationAllowedErr);
        }
        ...
        public override string this[ulong index]
        {
            get
            {
                ...
                var ind = (int)index; //Dictionary<string, CssCollectedProperty>
