...
{
    /// <summary>
    ///     The CSSStyleSheet interface is a concrete interface used to represent a CSS style sheet i.e., a style sheet whose
    ...
    public class CssStyleSheet : StyleSheet, ICssStyleSheet
    {
        ...
        {
            if (((MediaList)Media).Matches(ml)) ((CssRuleList)CssRules).GetStylesForElement(elt, pseudoElt, ml, csd);
        }
        ...
        private string PreProcessContent()
        {
            if (SheetContent != null && SheetContent.Length > 0)
            {
                ...
                var s = re.Replace(SheetContent, StringReplaceEvaluator);
        ...
        private readonly List<string> alReplacedStrings = new List<string>();
        ...
        ///     INDEX_SIZE_ERR: Raised if the specified index does not correspond to a rule in the style
        ///     sheet's rule list.
        /// </exception>
        ...
        /// </summary>
        /// <param name="rule">
        ///     The parsable text representing the rule. For rule sets this contains both the selector and the style
        ///     declaration. For at-rules, this specifies both the at-identifier and the rule content.
        /// </param>
        /// <param name="index">
        ///     The index within the style sheet's rule list of the rule before which to insert the specified rule.
        ///     If the specified index is equal to the length of the style sheet's rule collection, the rule will be added to the
        ...
        /// <exception cref="DomException">NO_MODIFICATION_ALLOWED_ERR: Raised if this style sheet is readonly.</exception>
        /// <exception cref="DomException">
        ///     HIERARCHY_REQUEST_ERR: Raised if the rule cannot be inserted at the specified index e.g.
        ///     if an @import rule is inserted after a standard rule set or other at-rule.
        /// </exception>
        ...
        private readonly CssRule ownerRule;

        /// <summary>
        ///     If this style sheet comes from an @import rule, the ownerRule attribute will contain the CSSImportRule. In that
        ///     case, the ownerNode attribute in the StyleSheet interface will be null. If the style sheet comes from an element or
        ///     a processing instruction, the ownerRule attribute will be null and the ownerNode attribute will contain the Node.
        /// </summary>
        public ICssRule OwnerRule => ownerRule;
