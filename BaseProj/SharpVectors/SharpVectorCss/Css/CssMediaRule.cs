...
{
    /// <summary>
    ///     The CSSMediaRule interface represents a @media rule in a CSS style sheet. A @media rule can be used to delimit
    ...
    public class CssMediaRule : CssRule, ICssMediaRule
    {
        ...
        /// </summary>
        public override CssRuleType Type => CssRuleType.MediaRule;
        ...
        {
            if (media.Matches(ml)) ((CssRuleList)CssRules).GetStylesForElement(elt, pseudoElt, ml, csd);
        }
        ...
        private static readonly Regex regex = new Regex(@"^@media\s(?<medianames>([a-z]+(\s*,\s*)?)+)");

        internal static CssRule Parse(ref string css, object parent, bool readOnly, string[] replacedStrings,
            CssStyleSheetType origin)
        {
            var match = regex.Match(css);
            if (match.Success)
            {
                var rule = new CssMediaRule(match, parent, readOnly, replacedStrings, origin);
        ...
        ///     INDEX_SIZE_ERR: Raised if the specified index does not correspond to a rule in the media
        ///     rule list.
        /// </exception>
        ...
        /// </param>
        /// <param name="index">
        ///     The index within the media block's rule collection of the rule before which to insert the specified
        ///     rule. If the specified index is equal to the length of the media blocks's rule collection, the rule will be added
        ///     to the end of the media block.
        /// </param>
        ...
        /// </summary>
        public ICssRuleList CssRules => cssRules;
