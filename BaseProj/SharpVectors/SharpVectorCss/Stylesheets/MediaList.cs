...
namespace BaseProj.SharpVectors.SharpVectorCss.Stylesheets
{
    ...
    ///     constraining how this collection is implemented. An empty list is the same as a list that contains the medium
    ///     "all".
    ///     The items in the MediaList are accessible via an integral index, starting from 0.
    /// </summary>
    public sealed class MediaList : IMediaList
    {
        private static readonly Regex splitter = new Regex(@"\s*,\s*");
        ...
        private void _parseString(string mediaText)
        {
            ...
            {
                var ms = splitter.Split(mediaText);
                foreach (var m in ms) AppendMedium(m);
            }
        }
        ...
        private bool containsAll; //speeds up matching if the list contains "all"
        private readonly List<string> medias = new List<string>();
        ...
        ///     INVALID_CHARACTER_ERR: If the medium contains characters that are invalid in the
        ...
        /// </summary>
        public ulong Length => (ulong)medias.Count;
        ...
        public string MediaText
        {
            get
            {
                ...
                foreach (var media in medias) result += media + ",";
        ...
        /// <summary>
        ///     Returns the indexth in the list. If index is greater than or equal to the number of media in the list, this returns
        ///     null.
        /// </summary>
        public string this[ulong index] => index < Length ? medias[(int)index] : null;
